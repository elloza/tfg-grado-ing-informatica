...
9. **Ortografía española antes de cerrar**. Si editas texto visible en español (`book/es/`, `_config_es.yml`, `_toc_es.yml`, `references.bib`, captions, alt text o skills), revisa tildes y signos de apertura en preguntas. Ejecuta `python scripts/check_spanish_orthography.py` y corrige palabras frecuentes sin tilde (`Documentación`, `Ingeniería`, `código`, `bibliografía`, etc.) antes de compilar, publicar o responder “listo”.
10. **Celdas de código ASCII-safe**. En notebooks, el Markdown puede llevar acentos, pero las celdas `code` deben usar ASCII en comentarios, `print()`, títulos/ejes/leyendas de Matplotlib y strings visibles. Usa `Ohm`, `pi`, `Delta`, `uF`, `tau`, `x^2` o mathtext (`r"$x^2$"`) en lugar de símbolos Unicode dentro del código.
11. **Assets web seguros**. PNG/JPG/SVG son formatos estables para HTML/PDF. No cambies referencias a WebP de forma general salvo que exista fallback PNG/JPG y se haya probado el PDF. Tras añadir imágenes o GIFs, ejecuta `python scripts/optimize_static_assets.py --check`; usa `--fix` solo para optimización conservadora de PNG/JPG.
